				...
				data = data.replace(/(^\s*)|(\s*$)/g, "");
				   if(data != 'success'){
					  ...
					  (type!='checkbox' && type!='radio' && element.addClass(validator.errorInput));
						if(msgid.length>0){
							msgid.html('&nbsp;&nbsp;<label class="btn-error js-error"><span><b class="ico-error"></b>'+errmsg+'</span></label>');
						} else{
							jQuery('&nbsp;&nbsp;<label class="btn-error js-error"><span><b class="ico-error"></b>'+errmsg+'</span></label>').insertAfter(element);
						}
						return false;
					}
