...
//标签页（下面无按钮的），参数n从0计数
function topWithBtn(n){
	//var topDiv = document.getElementById('tabBox').getElementsByTagName('span');
	
	var topDiv =$("#tabBox").find("span").not($(".tab_line"));
	
	for(var i=0;i<topDiv.length;i++){
		if($(topDiv[i]).attr("name")=='tabSpan'){
			$(topDiv[i]).attr("class","normal");
			$(topDiv[2*n]).attr("class","hover");
		}
	}
...
var s="";
var objs;
function startjudge(obj){
	objs=obj
	var oldval=$(obj).attr("value");
	var tablename=$(obj).attr("tablename");
	var tablecolid=$(obj).attr("tablecolid");
	var tablecolname=$(obj).attr("tablecolname");
	...
	s= setInterval(function(){
	   ...
	   if(oldval!=oldvaln){
		 oldval=oldvaln;
	       ...
	       autoComple(oldvaln,tablename,tablecolid,tablecolname,condition,obj,order);
...
/***自动补全方法*/
function autoComple(oldval,tablename,tablecolid,tablecolname,condition,obj,order){
 ...
 var orders="'"+order+"'";
 var mt=top+h;
 $(".bdsug").css({"display":"block","margin-top":top-80,"margin-left":left-97,"z-index": "999","width": wid+7,"overflow":"auto","position": "absolute"});
 /**正则判断汉字还是其他**/
     $.ajax({
		   type: "POST",
		      ...
		      var content='<ul class='+order+'>';
			   if(msg.length>0){
			       for(var i=0;i<msg.length;i++){
					  var val=msg[i].VAL;
					  var vals="'"+val+"'";
						content+="<li class='' val="+vals+" onclick=pay("+vals+","+orders+"); onmouseover=A(this); onmouseout=B(this);  >"+val+"</li>";
					}
			    }
			    content+='</ul>'
			     var jcontent = $(content);
			    $(".bdsug").empty();
				$(".bdsug").append(jcontent);
			  }
...
function display(max){
	...
	document.all.load_text.style.display = "block";
	var timer = window.setTimeout("display("+max+")",200);
	if (already >= max){
		window.clearTimeout(timer);
		document.all.load_text.innerHTML = "<font style='font-weight:bold;'>已完成。</font>";
     }
} 

//子页面调用父页面的方法，并跳转到另一页面
//参数jumptopageName：父页面要跳转的页面名称
function getOpener(jumptopageName){
	display(20);
	var param = window.dialogArguments
	if(param!=null && param!=""){
		param.getButtonView(jumptopageName);
	}
	closeit();
}
...
//继续弹窗
function getOpeners(str,str1){
	display(20);
	setTimeout(function(){ self.close(); window.dialogArguments.showModalDialog(str,window,str1);},5000);	
}